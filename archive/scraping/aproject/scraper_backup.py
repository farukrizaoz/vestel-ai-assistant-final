...
def _fetch(url: str, timeout: int = 30, method: str = "GET", allow_redirects: bool = True, max_retries: int = 3) -> requests.Response:
    ...
    if "vestel.com.tr" in url:
        headers["Referer"] = "https://www.vestel.com.tr/"
        headers["Origin"] = "https://www.vestel.com.tr"
        headers["Host"] = "www.vestel.com.tr"
    
    for attempt in range(max_retries):
        try:
            # Add progressive delay between attempts 
            if attempt > 0:
                delay = random.uniform(3, 8) * (attempt + 1)  # Longer delays
            ...
            # Use simple requests instead of session to avoid issues
            ...
            if "NameResolutionError" in error_msg or "Failed to resolve" in error_msg:
                print(f"    DNS resolution failed on attempt {attempt + 1}")
                # DNS failures might indicate temporary network issues
                time.sleep(random.uniform(10, 20))
            elif "Connection refused" in error_msg:
                print(f"    Connection refused on attempt {attempt + 1} - possible ban")
                time.sleep(random.uniform(30, 60))  # Longer wait for connection refused
            else:
...
def download_manual_pdf(manual_url: str, product_url: str) -> str:
    ...
    # Download the file
    try:
        from aproject.pdf_reader import download_file
        pdf_path = download_file(manual_url, file_basename=file_basename, timeout_sec=REQUEST_TIMEOUT)
...
def run_products(limit: int, skip_done: bool, verbose: bool):
    ...
    for i, url in enumerate(urls, 1):
        try:
            ...
            try:
                from aproject.pdf_reader import extract_pdf_text
                from aproject.analyzer import summarize_product_from_pdf
                
                text = extract_pdf_text(pdf_path)
                if text:
                    try:
                        analysis = summarize_product_from_pdf(pdf_path, title or "Ürün", url)
                        update_manual_analysis(
                            product_url=url,
                            keywords=analysis["keywords"],
                            description=analysis["short_desc"],
                            model_number=analysis.get("model_number", "")
                        )
                        if verbose:
                            print(f"  ✅ manual analyzed: {len(analysis['keywords'])} keywords, model: {analysis.get('model_number', 'N/A')}")
                    except Exception as e:
                        if verbose:
